...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext) {
		...
		if (new URL(request.url).pathname.startsWith("/v1/")) {
			...
			response =
				limited ?? withPublicApiRateHeaders(await startWorker.fetch(request));
		} else {
			response = await startWorker.fetch(request);
		}
	...
	async scheduled(event: ScheduledController, env: Env, ctx: ExecutionContext) {
		...
		const route =
			event.cron === "0 4 * * *" ? "/api/cron/maintenance" : "/api/cron/tasks";
		ctx.waitUntil(
