...
describe("parseCliArgs", () => {
  ...
  it("parses semantic search arguments", () => {
    expect(
      parseCliArgs(["search", "--query", "deployment decision", "--limit", "7", "--json"]),
    ).toEqual({
      kind: "invoke",
      method: "search",
  ...
  it("parses the stable SDK bridge command without shell interpretation", () => {
    ...
    expect(() =>
      parseCliArgs(["call", "unknown", "--input", "{}"]),
    ).toThrow("supported Desktop method");
  });

  it("accepts SDK payloads over stdin without putting source text in argv", () => {
    expect(
      parseCliArgs(["call", "documentIngest", "--input-stdin"]),
    ).toEqual({
      kind: "invoke",
