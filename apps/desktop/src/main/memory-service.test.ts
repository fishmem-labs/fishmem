...
describe("DesktopMemoryService local-only embedding contract", () => {
	...
	it("rejects add and search until local E5 is ready instead of using keyword fallback", async () => {
		...
		await expect(service.invoke("history", { id })).resolves.toMatchObject({
			results: [{ event: "ADD" }, { event: "UPDATE" }, { event: "UPDATE" }],
		});
		await expect(
	...
	it("exports portable snapshots and restores the whole library without partial replacement", async () => {
		directory = await mkdtemp(join(tmpdir(), "fishmem-desktop-backup-test-"));
		const embedder = new ControlledLocalEmbedder();
		service = new DesktopMemoryService(
			join(directory, "fishmem.db"),
			join(directory, "models"),
		...
		await service.initialize();
		const ready = service.invoke("retryEmbedding");
		embedder.release();
		await ready;

		await service.invoke("add", {
			content: "Keep this memory in the backup",
			idempotency_key: "backup-source-memory",
		});
		const snapshot = await service.invoke("exportSnapshot");
		await service.invoke("add", {
			content: "Created after the backup",
			idempotency_key: "post-backup-memory",
		});

		await expect(
			service.invoke("restoreSnapshot", { snapshot }),
		).rejects.toThrow('requires confirm: "RESTORE"');
		await expect(
			service.invoke("restoreSnapshot", { snapshot, confirm: "RESTORE" }),
		).resolves.toMatchObject({ imported: 1, vectors: 1 });

		const restored = (await service.invoke("list", { limit: 10 })) as {
			results: Array<{ content: string }>;
		};
		expect(restored.results.map((memory) => memory.content)).toEqual([
			"Keep this memory in the backup",
		]);

		await expect(
			service.invoke("restoreSnapshot", {
				snapshot: { format: "not-fishmem" },
				confirm: "RESTORE",
			}),
		).rejects.toThrow("unsupported fishmem snapshot format");
		const unchanged = (await service.invoke("list", { limit: 10 })) as {
			results: Array<{ content: string }>;
		};
		expect(unchanged.results.map((memory) => memory.content)).toEqual([
			"Keep this memory in the backup",
		]);
	});
});
