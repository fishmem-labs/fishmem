...
function Memories({
	...
	const load = useCallback(async () => {
		...
		try {
			const page = submittedQuery
				...
				: await window.fishmem.invoke<DesktopMemoryPage>("list", {
						limit: 100,
					});
	...
	return (
		...
		>
			...
			<MemoryInspectorDrawer
				hasNext={selectedIndex >= 0 && selectedIndex < visibleRows.length - 1}
				hasPrevious={selectedIndex > 0}
				history={history}
				historyLoading={historyLoading}
				memory={selectedMemory ? toDashboardMemory(selectedMemory) : undefined}
				onClose={() => setSelectedId(null)}
				onNext={() => setSelectedId(visibleRows[selectedIndex + 1]?.id ?? null)}
				onPrevious={() =>
			...
			{status && (
				<Panel title="FishMem command">
					...
					<p className="mt-2 text-xs text-muted-foreground">
						Both skills use this local command to communicate with FishMem
						Desktop.
					</p>
				</Panel>
			)}
...
function SettingsPanel({
	...
	async function exportBackup() {
		...
		try {
			const result =
				await window.fishmem.invoke<DesktopBackupResult>("exportBackup");
			if (!result.canceled) {
				setMessage(`Backup exported to ${result.path ?? "the selected file"}.`);
			}
		} catch (cause) {
			setMessage(cause instanceof Error ? cause.message : String(cause));
		} finally {
	...
	async function restoreBackup() {
		...
		try {
			const result =
				await window.fishmem.invoke<DesktopBackupResult>("restoreBackup");
			if (!result.canceled) {
				setMessage(
					`Backup restored: ${result.imported ?? 0} memories and ${result.documents ?? 0} documents.`,
				);
				await onSaved();
			}
		} catch (cause) {
	...
	return (
		<>
			<PageShell
				title="Settings"
				description="Inspect local storage, export a portable backup, or restore your library."
			>
				<div>
				...
				<div>
					...
					<p className="mt-1 text-sm text-muted-foreground">
						Backups contain authoritative memories, history, entities, source
						documents, and events. Search indexes are rebuilt after restore.
					</p>
				</div>
				<Panel title="Portable FishMem snapshot">
					<div className="flex flex-wrap items-center justify-between gap-4">
						<p className="max-w-xl text-sm leading-relaxed text-muted-foreground">
							Export before upgrades or moving devices. Restore validates the
							full snapshot first and rolls the current library back if import
							fails.
						</p>
						<div className="flex flex-wrap gap-2">
							...
							<button
								className={dashButton.outline}
								disabled={
									backupBusy !== null || status?.embeddingState !== "ready"
								}
			...
			<ConfirmDialog
				confirmLabel="Choose backup and restore"
				confirmText="RESTORE"
				description="This replaces every memory and source in the current local library. FishMem validates the backup before deletion and attempts an automatic rollback if restore fails."
				destructive
				loading={backupBusy === "restore"}
				onClose={() => setRestoreOpen(false)}
				onConfirm={() => void restoreBackup()}
				open={restoreOpen}
				title="Replace the local FishMem library?"
			/>
		</>
	);
}
