  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        console.log(
          "Validated packaged FishMem startup and embedded CLI status",
        );
        return;
      } catch (error) {
        lastError = error;
    ...
    throw new Error(
      `Packaged FishMem did not become ready within 60 seconds: ${lastError instanceof Error ? lastError.message : String(lastError)}\n${stdout}${stderr}`,
    );
  } finally {
    if (child.exitCode === null && child.signalCode === null) {
      child.kill("SIGTERM");
...
async function validateNativeArchitectures(appPath) {
  ...
  if (nativeFiles.length === 0) {
    throw new Error("No packaged native modules were found for architecture checks");
  }

  for (const nativeFile of nativeFiles) {
    const normalized = nativeFile.replaceAll("\\", "/");
    if (normalized.includes("/linux/") || normalized.includes("/win32/")) {
      throw new Error(`Non-macOS native runtime leaked into the app: ${nativeFile}`);
    }
    if (
...
async function validateApp(appPath, { fullNativeCheck = false } = {}) {
  ...
  if (!signatureDetails.includes(`Authority=${identity}`)) {
    throw new Error(`FishMem.app is not signed by the expected identity: ${identity}`);
  }
  ...
  const mainExecutable = join(appPath, "Contents", "MacOS", productName);
  const cliExecutable = join(appPath, "Contents", "Resources", "bin", "fishmem");
