...
describe("MemoryApplication", () => {
	...
	it("forwards an explicit event date into the canonical engine", async () => {
		const { app, memory } = fixture();
		...
		expect(memory.add).toHaveBeenCalledWith(
			"Alex moved to Berlin in July 2026.",
	...
	it("lists, gets, paginates, and retry-safely deletes scope entities", async () => {
		...
		await expect(
			app.deleteScopeEntity("workspace", "agent", "helper", "delete-helper-1"),
		).resolves.toEqual(deleted);
		await expect(
			app.deleteScopeEntity("workspace", "agent", "helper", "delete-helper-1"),
		).resolves.toEqual(deleted);
		await expect(
			app.deleteScopeEntity("workspace", "agent", "helper", "delete-helper-2"),
	...
	it("runs the canonical CRUD and operation journey on SQLite", async () => {
		...
		expect(
			(await app.search("workspace", { query: "tea", user_id: "ada" })).results,
		).not.toHaveLength(0);
		expect(
		...
		expect(
			(await app.history("workspace", id)).map((row) => row.event),
		).toEqual(["ADD", "UPDATE"]);
