...
function searchOptions(
  ...
  return {
    ...
    ...(command.min_score !== undefined
      ? { minScore: command.min_score }
      : {}),
    ...(command.filters
...
function isAdvancedFilter(
  ...
  return (
    "field" in value || "and" in value || "or" in value || "not" in value
  );
}
...
function compileFilter(
  ...
  const value = Array.isArray(expression.value)
    ? expression.value.map((item) => (dateField ? new Date(String(item)) : item))
    : dateField
...
function decodeScopeEntityCursor(cursor?: string):
...
export class MemoryApplication {
  ...
  async listScopeEntities(namespaceId: string, input: unknown) {
    ...
    return {
      ...
      next_cursor:
        hasMore && last ? encodeScopeEntityCursor(last) : null,
    };
  }
  ...
  async getOperation(namespaceId: string, operationId: string) {
    const operation = await this.namespace(namespaceId).getOperation(operationId);
    if (!operation) {
      throw new MemoryApplicationError(
        "OPERATION_NOT_FOUND",
        "Operation not found",
  ...
  async getState(namespaceId: string, query: unknown) {
    ...
    return this.namespace(namespaceId).getState(
      command.subject,
      command.attribute,
      { ...scope(command), asOf: command.as_of ? new Date(command.as_of) : undefined },
    );
  }
