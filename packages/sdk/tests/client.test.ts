...
describe("FishMem universal client", () => {
  ...
  it("sends typed retrieval controls in the search body", async () => {
    ...
    expect(String(url)).toBe(
      "https://memory.example/v1/memories/search",
    );
  ...
  it("lists, pages, reads, and idempotently deletes structural scope entities", async () => {
    ...
    const fetch = vi
      ...
      .mockResolvedValueOnce(
        json({ results: [second], next_cursor: null }),
      )
  ...
  it("uploads exact source bytes through the asynchronous asset lifecycle", async () => {
    ...
    expect(
      new Headers(createInit?.headers).get("Idempotency-Key"),
    ).toBe("upload-1");
    ...
    expect(
      new Headers(contentInit?.headers).get("Idempotency-Key"),
    ).toBeNull();
    expect(
      new Headers(contentInit?.headers).get("Authorization"),
    ).toBeNull();
    expect(
  ...
  it("covers the complete memory, state, profile, and operation surface", async () => {
    ...
    await expect(
      client.profile.get({ user_id: "ada" }),
