...
- Provider-level LLM/embedding calls, response token usage, and reference USD
  are instrumented through a scoped transport meter. Unit usage is saved in
  checkpoints so resume/cache cannot undercount. The publication gate rejects
  unscoped, unmetered, unexpectedly-unpriced, or differently-priced paired runs;
  a version-pinned ChatGPT-subscription Codex answer is the sole exception and
  must occur exactly once per evaluated item, with USD explicitly excluded.
...
- LOCOMO, LongMemEval, and BEAM share an answer-provider seam. Responses API
  runs are reproducible evidence candidates. Subscription-backed Codex
  app-server runs are also candidates when they freeze exact model/runtime,
  stateless no-tools isolation, and instruction hash; other Codex transports
  remain development canaries.
- The frozen 2026-08-24 public portfolio now covers all 1,986 LOCOMO questions,
  all 500 LongMemEval `oracle` items, and the 400-question BEAM `100k` slice.
  It passes the quality gate with significant wins on LongMemEval and BEAM and
  a significant LOCOMO loss. The immutable sources and caveats are linked from
  `benchmarks/reports/evidence-2026-08-24.md`.
- Aggregate cost and end-to-end wall-clock claims remain blocked until a clean
  isolated rerun replaces the resumed LongMemEval and BEAM mem0 baselines.
  Context efficiency is also still open: the winning FishMem runs retrieved
  substantially more context than mem0.
...
- same answer model
- same write/extraction model and reasoning effort
- same answer provider, reasoning effort, transport, and runtime version
- same judge model
- same answer prompt
- same judge prompt
...
- homepage/README headline evidence uses the full split, not a smoke/dev subset
- homepage/README portfolio includes at least all 1,986 LOCOMO questions, all
  500 LongMemEval questions, and 400 BEAM questions, with every public category
  represented; smaller samples remain canary/draft evidence only
...
- recovered provider failures remain disclosed and must stay at or below 2% of
  all attempts; exhausted writes or judge calls never enter the sample
- warning counts, retries, timeouts, and total degradation count recorded
- result JSON committed or archived with exact config
- answer-provider manifest does not mark the run non-publishable
